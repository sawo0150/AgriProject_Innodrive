...
class MapOdomTFPublisher(Node):
    ...
    def load_map_points(self, yaml_file):
        ...
        occupancy map에서 장애물 내부가 아닌 경계선 부분만 추출하여,
        월드 좌표계의 2D 점들로 변환한다.
        """
        try:
            ...
            # 이진 occupancy 이미지 생성: 장애물 픽셀은 255, 자유공간은 0
            binary = (occ_data < threshold).astype(np.uint8) * 255
            
            # 경계선 추출: morphological gradient로 장애물 경계 검출
            kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (2, 2))
            edges = cv2.morphologyEx(binary, cv2.MORPH_GRADIENT, kernel)
            # 경계선을 두껍게: dilate 연산
            dilated_edges = cv2.dilate(edges, kernel, iterations=2)
            
            # 경계선 이미지 디버깅용으로 출력 (매 프레임마다 창 업데이트)
            # cv2.imshow("Map Boundary", dilated_edges)
            # cv2.waitKey(100)
            
            # 경계선 픽셀 좌표 추출 (행, 열)
            boundaries = np.where(dilated_edges > 0)
            points = []
            height = map_array.shape[0]
            for row, col in zip(boundaries[0], boundaries[1]):
                # ROS map_server와 같이 (0,0)은 왼쪽 위이므로 y 좌표 반전 적용
                x = col * resolution + origin[0]
                y = (height - row) * resolution + origin[1]
                points.append([x, y])
            points = np.array(points)
            
            self.get_logger().info(f"Loaded map boundaries with {points.shape[0]} points from {yaml_file}.")
            return points
        except Exception as e:
