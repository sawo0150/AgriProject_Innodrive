...
class CubicSpline1D: # 여러 점들에 대해서 4차 함수형태로 부드러운 연결을 도와주는 class
    """
    ...
    """
#spline 함수 형태(i번때 점마다 하나씩 함수가 존재함) : si(x) = a + bi(x-xi) + ci(x-xi)^2 + di(x-xi)^3
#각 구간에서 매끄러운 다항식 Si(x) 생성하기 (2차 미분까지 연속)
#만족해야할 조건 : Si(xi)=yi, Si(xi)=yi+1 / 1차 미분 연속성, 2차 미분 연속성 / 양쪽 끝에서 2차 미분값 0이어야함
# 이조건을 행렬 식형태로 만듦 (A.c = B) - A는 spline 계수 계산 행렬 / B는 y, 
    def __init__(self, x, y): #x는 경로의 길이 s에 해당하고, y는 x나 y를 의미함.
