...
from frenet_local_pathplanning.include.quintic_polynomials_planner import QuinticPolynomial
...
LATERAL_MOVEMENT = LateralMovement.LOW_SPEED
LONGITUDINAL_MOVEMENT = LongitudinalMovement.VELOCITY_KEEPING
# LONGITUDINAL_MOVEMENT = LongitudinalMovement.MERGING_AND_STOPPING
...
if LATERAL_MOVEMENT == LateralMovement.LOW_SPEED:
    ...
    OBSTACLES = np.array([[3.0, 1.0], [7.0, 0.0], [5.0, -0.0], [6.0, 0.5], [8.0, -1.5]])
...
def calc_frenet_paths(c_s_d, c_s_dd, c_d, c_d_d, c_d_dd, s0):
    ...
    for Ti in np.arange(MIN_T, MAX_T, DT): # -> 각 Ti초마다 도달할 수 있는 다양한 경로 생성함
        lon_paths = LONGITUDINAL_MOVEMENT_STRATEGY.calc_longitudinal_trajectory( #종방향 후보 경로 생성함
            c_s_d, c_s_dd, Ti, s0
        )

        for fp in lon_paths:
            for di in LONGITUDINAL_MOVEMENT_STRATEGY.get_d_arrange(s0): #횡방향 road 범위 받기
                tp = LATERAL_MOVEMENT_STRATEGY.calc_lateral_trajectory( # 횡방향 road 범위 받기
...
def generate_target_course(x, y):
    ...
    s = np.arange(0, csp.s[-1], 0.1) #csp.s는 누적 거리를 의미함 (각 지점까지 경로 거리를 의미함)

    rx, ry, ryaw, rk, rdk = [], [], [], [], []
    for i_s in s:   #s를 기준으로 0.1m 마다 taget course를 만드는 거임
        ix, iy = csp.calc_position(i_s) #spline 보간을 통해서 
        rx.append(ix)
        ry.append(iy)
        ryaw.append(csp.calc_yaw(i_s))
        rk.append(csp.calc_curvature(i_s)) #rk는 곡률을 의미함 <- 이걸로 steering 계산 쉽게 할 수 있을듯?
        rdk.append(csp.calc_curvature_rate(i_s))


    return rx, ry, ryaw, rk, rdk, s, csp
